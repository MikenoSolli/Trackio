...
export async function POST() {
  try {
    ...
    try {
      await prisma.$executeRawUnsafe(`ALTER TABLE users ADD COLUMN companyId INT NULL`);
    ...
    try {
      await prisma.$executeRawUnsafe(`ALTER TABLE vehicle ADD COLUMN companyId INT NULL`);
    } catch (e) { /* column may already exist */ }
    try {
    ...
    try {
      await prisma.$executeRawUnsafe(`ALTER TABLE driver ADD COLUMN companyId INT NULL`);
    } catch (e) { /* column may already exist */ }
    try {
    ...
    try {
      await prisma.company.createMany({
        data: [
          { name: "TractorTrack Inc.", email: "admin@tractortrack.com", phone: "+1234567890", address: "123 Farm Road" },
          { name: "Farm Solutions Ltd.", email: "info@farmsolutions.com", phone: "+1234567891", address: "456 Agri Street" },
          { name: "Heavy Machinery Co.", email: "contact@heavymachinery.com", phone: "+1234567892", address: "789 Industrial Park" },
          { name: "Fleet Management Group", email: "support@fleetmgmt.com", phone: "+1234567893", address: "321 Logistics Ave" },
        ],
        skipDuplicates: true,
      });
